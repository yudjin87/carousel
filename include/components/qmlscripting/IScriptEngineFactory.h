...
/*!
 * @brief
 *   This interface describes an abstract factory for the QJSEngine.
 * @details
 *   Because ScriptUnit should create a new QJSEngine each time it
 *   executes a script (to run it from white board, with clean context),
 *   it is needed some kind of the QJSEngine factory, because just instantiate
 *   a new instance of the QJSEngine is not enough - an engine should be
 ...
 */
class QML_SCRIPTING_API IScriptEngineFactory
{
public:
    /*!
     * @details
     ...
     */
    virtual QJSEngine *createEngine(IOutputHandler *output = nullptr, QObject *parent = nullptr) = 0;
